import { DefenseResource } from '../../worker/api';
...
const personnelImages: Record<string, string> = {
...
// Troop mapping
const troopImages: Record<string, string> = {
...
// Squadron mapping
const squadronImages: Record<string, string> = {
  'a_wing': awing,
  'b_wing': bwing,
  'x_wing': xwing,
  'y_wing': ywing,
...
// Shield mapping
const shieldImages: Record<string, string> = {
  ...
  switch (resource.type) {
    case 'personnel':
      if (resource.subtype === 'characters') {
        ...
        return characterImages[resource.id] || characterImages['luke_skywalker']; // fallback
      } else {
        // For other personnel, use the subtype
        return personnelImages[resource.subtype] || personnelImages['bothan_spies']; // fallback
      }
    
    case 'troop':
      return troopImages[resource.subtype] || troopImages['alliance_army_regiment']; // fallback
    ...
    default:
      return characterImages['luke_skywalker']; // default fallback
  }
}
