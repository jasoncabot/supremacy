...
export const TabGroupComponent: React.FC<TabGroupComponentProps> = ({
	...
	return (
		...
		>
			<TabList className="flex flex-shrink-0 space-x-1 rounded-md bg-slate-800">
				{tabs.map((tab) => {
					...
					return (
						...
						>
							{iconOnly ? (
								...
								<>
									{TabIcon && <TabIcon className="mr-1 h-6 w-6" />}
			...
			<TabPanels className="mt-2 flex-1 overflow-hidden">
				{tabs.map((tab) => (
					<TabPanel key={tab.id} className="h-full overflow-y-auto rounded-md">
						{tab.content}
					</TabPanel>
				))}
			</TabPanels>
